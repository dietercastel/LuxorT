...
function getLogistic(mx,startp,stepSize)
	midpoint = 0.5
	growthrate = 1
	logistic = x-> mx /(1+(ℯ^(-growthrate*(x-midpoint))))
	pts = (-startp):stepSize:startp
	len = length(pts) 
	x = vcat(pts,repeat([0.0],len))
	println(x)
	y = vcat(repeat([0.0],len),pts)
	z = vcat(map(logistic,x[1:len]),-map(logistic,y[len+1:end]))
	xyz = [ [a[1];a[2];a[3]] for a in zip(x,y,z)]
	return ("logistic-$mx-$stepSize",xyz)
end
